...
public class MainHelper {
  ...
  public static Student chooseStudent(Scanner sc, College oth) {
		...
		/* example
		 * 1- Max Mustermann: 1234[Matrikelnummer]
		 * 2- Lillie Liesbert: 5678[Matrikelnummer]
		 */
		...
		for (Student student : oth.getStudents()) {
			System.out.println(
        /* #learnmore
         * wonder what j++ makes in the println here?
         * j++ increments the number after the operation. that means, after
         * we print the first student ID, we automatically increment i.
         * there is also ++i, which increments i BEFORE the operation.
         *
         * example:
         * i = 0, j = 0
         * i = ++i // i = 1
  ...
  public static Course chooseCourse(Scanner sc, College oth) {
    /* prints the following (for example)
     * 1: Wirtschaftsinformatik (Bachelor)
     * 2: Allgemeine Informatik (Bachelor)
     */
    int i = 1;
		for (Course course : oth.getCourses()){
			System.out.println(
				Integer.toString(i++) + ": " + course.getName() + " ("+course.getDegree()+")"
			);
		}
